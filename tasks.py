...
def coverage(c):
    ...
    """
    c.run("coverage run --source healthy_django runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/healthy_django.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ healthy_django")
...
def lint(c):
    ...
    """
    c.run("flake8 healthy_django tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=healthy_django.__version__))
    c.run("git push --tags")
